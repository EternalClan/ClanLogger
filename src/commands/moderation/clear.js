...
module.exports = {
	...
	async execute(message, args, prefix, commandName, globalclient) {
		...
		|| message.guild.id == null) return console.log(`[${DateTime.utc().toFormat(timeFormat)}][command.clear.js] Interaction returned null or undefined.`);
		// Data Get
		...
		if (isNaN(stringAmount)) {
			Utils.messageReply(message, { content: langClear.notanumber });
			return;
		}
		const guild = await message.client.guilds.fetch(getGuildID);
		const channel = await guild.channels.fetch(getChannelID);
		if (stringAmount > 100) {
			Utils.messageReply(message, { content: langClear.max, ephemeral: true });
		}
		if (stringAmount <= 100) {
