...
module.exports = {
	...
	async execute(message, args, prefix, commandName, globalclient) {
		// // Interaction
		// if (message.constructor === BaseInteraction) {
		// 	//
		// }
		// // Message
		...
		if (message == null) return console.log(`[${DateTime.utc().toFormat(timeFormat)}][command.help.js] Interaction returned null or undefined.`);
		// Imports
		...
		if (!Array.isArray(data_new)) fetch_dm_user.send({ content: data_new }).catch(async () => console.info(`[command.help.js] User <${fetch_dm_user.id}> is not allowing DMs.`));

		if (Array.isArray(data_new)) {
			data_new.forEach(async (item) => {

				if (item.length >= 1999) console.info("[command.help.js] help.txt: Text Paragraph too long.");

				fetch_dm_user.send({ content: item }).catch(async () => console.info(`[command.help.js] User <${fetch_dm_user.id}> is not allowing DMs.`));
				// await sleep(1000);
			});
		}
	}
