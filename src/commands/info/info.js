...
module.exports = {
	...
	async execute(message, args, prefix, commandName, globalclient) {
		...
		|| message.guild.id == null) console.log(`[${DateTime.utc().toFormat(timeFormat)}][command.info.js] Interaction returned null or undefined.`);
		// Imports
		const { DevCheck, Utils } = require("../../tools/utils.js");
		...
		// @Bot info help
		// @Bot info bot
		// @Bot info member <member|memberID>
		// @Bot info channel <channel|channelID>
		// @Bot info commands
		// @Bot info
		...
		if (args[0] === "bot") {
			infoEmbed.setTitle(langInfo.titlebot)
				...
				.addFields([
					...
					// { name: '\u200B', value: '\u200B' },
					{ name: langInfo.prefix, value: "ㅤ@Bot", inline: true },
					{ name: langInfo.id, value: `${globalclient.user.id}`, inline: true }
				]);
			Utils.messageReply(message, { embeds: [infoEmbed] });
		}
		// Member infos
		if (args[0] === "member") {
